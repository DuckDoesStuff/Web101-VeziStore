...
const authService = require("./auth-admin.service");
...
const signin = (req, res, next) => {
	res.render("admin/auth/adminLogin", { title: "Sign In", showSideBar: false, mode: true});
};
...
const login = (req, res, next) => {
	passport.authenticate("local", (err, user, info) => {
		...
		if (!user) {
			...
			return res.json({
				msg: "Your account is not an admin. Please uses another account.",
			});
		}
...
const activate = async (req, res, next) => {
	...
	if(!result.email || result.email !== req.user.email) {
		return res.json({
			msg: "Invalid token.",
		});
	}
